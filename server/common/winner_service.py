...
class WinnerService:
    ...
    def get_winners_response(self, message: str):
        ...
        contestants = self.get_contestants(message)
        winners = self.get_winners(contestants)
        logging.debug("Get winners response. Winners: {}".format(winners))
        self.save_winners(winners)
        response = self.get_winners_string(winners)
        logging.info("Get winners response. Response: {}".format(response))
        return response

    def get_contestants(self, message: str):
        logging.info("Get Contestants.".format(message))
        logging.debug("Get Contestants. MSG: {}".format(message))
        player_list = self.parse_batch_message(message)
        contestant_list = []
    ...
    def parse_batch_message(self, batch_message: str):
        parsed = batch_message.split(BATCH_SEPARATOR)
        return parsed


    def parse_contestant_message(self, contestant_message: str):
    ...
    def get_winners(self, contestants: 'list[Contestant]'):
    ...
    def get_winners_string(self, winners: 'list[Contestant]'):
        string = ""
